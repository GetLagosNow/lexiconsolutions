	...
	class Lbrowserc_Info {
		...
		public function top_msg() {
			...
			if( ! get_user_meta( $user_id, 'lbrowserc_ignore_notice' ) ) {
				...
				printf( __( 'Using new WordPress Editor? Let\'s try <a href="%1$s">Di Blocks Plugin</a> | <a href="%2$s">Hide it</a>', 'lbrowserc' ), esc_url( $installation_link ), esc_url( add_query_arg( 'lbrowserc_ignore_notice', '0' ) ) );
